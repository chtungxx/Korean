[!DOCTYPE html.html](https://github.com/user-attachments/files/25891516/DOCTYPE.html.html)
<!DOCTYPE html>  
<html lang="zh-HK">  
<head>  
  <meta charset="UTF-8">  
...
<body class="bg-slate-50 font-sans text-slate-800 h-[100dvh] flex flex-col md:flex-row overflow-hidden">  
  ...
  <!-- 主要內容區 (Mobile: 移到上方優先顯示, Desktop: 佔滿右側) -->  
  <main class="flex-1 p-4 md:p-8 overflow-y-auto w-full relative order-1 md:order-2" id="main-content">  
    <!-- 內容由 JS 動態生成 -->  
  </main>  
  
  <!-- 側邊/底部導航欄 (Mobile: 固定在底部, Desktop: 固定在左側) -->  
  <nav class="w-full md:w-64 bg-white shadow-[0_-4px_10px_rgba(0,0,0,0.05)] md:shadow-lg flex md:flex-col justify-around md:justify-start p-2 md:p-4 shrink-0 z-50 order-2 md:order-1 pb-[calc(0.5rem+env(safe-area-inset-bottom))] md:pb-4 border-t md:border-t-0 md:border-r border-slate-100">  
  ...
  <script>  
    ...
    function renderWriting(container) {  
      ...
      let answerHtml = answer.map((item, idx) => {  
        ...
        return `  
          <div onclick="handleWriteAnswerClick(${idx})" class="w-12 h-12 sm:w-14 sm:h-14 md:w-20 md:h-20 flex items-center justify-center text-2xl md:text-3xl font-bold rounded-xl transition-all ${boxClass}">  
            ${item ? item.char : ''}  
          </div>  
        `;  
      }).join('');  
      ...
      if (status === 'playing') {  
        poolHtml = `  
          <div class="flex flex-wrap justify-center gap-2 sm:gap-3 mb-8">  
            ${pool.map((item, idx) => `  
              <button onclick="handleWritePoolClick(${idx})" class="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 bg-white border-2 border-slate-200 rounded-xl text-xl md:text-2xl font-bold text-slate-700 hover:border-indigo-400 hover:text-indigo-600 hover:-translate-y-1 transition-all shadow-sm">  
                ${item.char}  
              </button>  
            `).join('')}  
          </div>  
